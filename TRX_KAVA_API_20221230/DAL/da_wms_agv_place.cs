...
{
    public class da_wms_agv_place
    { //---------------------按Table的方式查询方法--------------------------//
        private static List<wms_agv_place> DoTableQuery(string sql_str, SqlParameter[] paras)
        {
            var lisRtn = new List<wms_agv_place>();
            try
            {
                var dtback = DBHelper_SQLServer.ExecuteDataTable(sql_str, paras);
                lisRtn = EntityTableParse<wms_agv_place>.ToList(dtback);
            }
            catch
            {
            }
        ...
        private static List<wms_agv_place> DoQuery(string sql_str, SqlParameter[] paras)
        {
            var lisRtn = new List<wms_agv_place>();
            try
            {
        ...
        //----------------------Insert方法----------------------------------------------//
        public static bool InsertNew(wms_agv_place newOne)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(" Insert into wms_agv_place (  ");
            sb.Append("plid, ");
        ...
        //----------------------UpdateOne方法---------------------------------------//
        public static bool UpdateOne(wms_agv_place newOne)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(" update wms_agv_place set  ");
        ...
        public static bool DeleteByPKID(object PK_ID)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append(" delete from wms_agv_place where plid=@plid ");
                SqlCommand cmd = new SqlCommand();
                cmd.CommandText = sb.ToString();
                cmd.CommandType = CommandType.Text;
        ...
        //----------------------GetOneByID方法--------------------------------------//
        public static wms_agv_place GetOneByID(object PK_ID)
        {
            List<SqlParameter> prs = new List<SqlParameter>();
            prs.Add(new SqlParameter("@plid", SqlDbType.NVarChar, 150) { Value = PK_ID });
            var lis = DoTableQuery("select * from wms_agv_place where plid =@plid", prs.ToArray());
        ...
        internal static wms_agv_place GetOneByCode(string placecode)
        {
            List<SqlParameter> prs = new List<SqlParameter>();
            prs.Add(new SqlParameter("@place_code", SqlDbType.NVarChar, 150) { Value = placecode });
            var lis = DoTableQuery("select * from wms_agv_place where place_code =@place_code", prs.ToArray());
            if (lis != null && lis.Count > 0)
            {
                return lis[0];
            }
        ...
        internal static wms_agv_place GetOneByPallet(string pallet)
        {
            List<SqlParameter> prs = new List<SqlParameter>();
            prs.Add(new SqlParameter("@pallet_container", SqlDbType.NVarChar, 150) { Value = pallet });
            var lis = DoTableQuery("select * from wms_agv_place where pallet_container =@pallet_container", prs.ToArray());
            if (lis != null && lis.Count > 0)
            {
                return lis[0];
            }
        ...
        /// </summary>
        /// <returns></returns>
        internal static List<wms_agv_place> Get(List<KVP> keyValues)
        {
            StringBuilder sb = new StringBuilder();
            List<SqlParameter> parametrs = new List<SqlParameter>();
            sb.Append("select * from wms_agv_place where  flag_delete = 0 ");
        ...
        internal static List<wms_agv_place> GetNotEmptyOrHasTaskPlaces()
        {
            StringBuilder sb = new StringBuilder();
            List<SqlParameter> parametrs = new List<SqlParameter>();
            sb.Append("select * from wms_agv_place where  flag_delete = 0 and flag_has_task ='Y' or is_empty ='N'");
            var rtn = DoTableQuery(sb.ToString(), parametrs.ToArray());
        ...
        internal static bool updateSeq(int seqs, int row, int column, int layer)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append(" update  wms_agv_place set numb_unit= " + seqs + " where numb_row= " + row + " and numb_column =" + column + " and numb_layer= " + layer);
                SqlCommand cmd = new SqlCommand();
                cmd.CommandText = sb.ToString();
                cmd.CommandType = CommandType.Text;
                int val = DBHelper_SQLServer.ExecuteNonQuery(cmd);
        ...
        internal static wms_agv_place PlaceGetOneEmptyByPallet(string name, string container_id)
        {
        ...
        internal static wms_agv_place PlaceGetOneEmptyByPallet1(string container_id)
        {
            List<SqlParameter> parass = new List<SqlParameter>();
            ...
            parass.Add(pa);
            var prrslt = parass.ToArray();
            var rslt = DAL.DBHelper_SQLServer.ExecuteProcedure("p_get_in_empty_place_by_pallet_Highter", ref prrslt);
            var returnVL = pa.Value.ToString();
            var str = "执行存储过程 p_get_in_empty_place_by_pallet_Highter, 返回行值" + rslt + "结果:" + returnVL;
            LogHelper.Info(str);
        ...
        internal static wms_agv_place PlaceGetOneEmptyByPallet2(string container_id)
        {
            List<SqlParameter> parass = new List<SqlParameter>();
        ...
        internal static string InOccupyPlace(string task, string placenum)
        {
            try
            {
                ...
                parass.Add(pa);
                var prrslt = parass.ToArray();
                var rslt = DAL.DBHelper_SQLServer.ExecuteProcedure("p_srm_in_agv_task_occupy_place", ref prrslt);
        ...
        internal static string InFinishUpdate(string task, string placenum, string palletnum)
        {
            try
            {
                ...
                parass.Add(pa);
                var prrslt = parass.ToArray();
                var rslt = DAL.DBHelper_SQLServer.ExecuteProcedure("p_agv_in_task_finish_update", ref prrslt);
                var outResult = prrslt.ToList().Find(p => p.ParameterName == "@return_value");
                rtn = (outResult == null || outResult.Value == null) ? "E执行失败原因未知" : outResult.Value.ToString();
        ...
        internal static string AGVtaskfinished(string taskcode)
        {
            try
            {
                string rtn = "E初始化未提交数据库";
                ...
                parass.Add(new SqlParameter("@taskcode", taskcode));
                SqlParameter pa = new SqlParameter();
                pa.Direction = ParameterDirection.InputOutput;
                pa.ParameterName = "@return_value";
                pa.Size = 5000;
                ...
                parass.Add(pa);
                var prrslt = parass.ToArray();
                var rslt = DAL.DBHelper_SQLServer.ExecuteProcedure("p_srm_agv_transfer_finish_update", ref prrslt);
                var outResult = prrslt.ToList().Find(p => p.ParameterName == "@return_value");
                rtn = (outResult == null || outResult.Value == null) ? "E执行失败原因未知" : outResult.Value.ToString();
                var str = "执行存储过程 p_srm_agv_transfer_finish_update, 任务号：" + taskcode + ",结果" + rtn;
                LogHelper.Info(str);
                return rtn;
            }
            catch (Exception ec)
            {
                var str = "E执行存储过程 p_srm_agv_transfer_finish_update" + ec.Message + "\r\n" + ec.StackTrace;
                LogHelper.Error(str);
                return str;
            }
        }

        internal static string InOccupyPlace(string task, string placefrom, string placenum)
        {
            try
            {
                string rtn = "E初始化未提交数据库";
                List<SqlParameter> parass = new List<SqlParameter>();
                parass.Add(new SqlParameter("@taskcode", task));
                parass.Add(new SqlParameter("@place_code", placefrom));
                parass.Add(new SqlParameter("@from", placenum));
                SqlParameter pa = new SqlParameter();
                pa.Direction = ParameterDirection.InputOutput;
                pa.ParameterName = "@return_value";
                pa.Size = 5000;
                ...
                var outResult = prrslt.ToList().Find(p => p.ParameterName == "@return_value");
                rtn = (outResult == null || outResult.Value == null) ? "E执行失败原因未知" : outResult.Value.ToString();
                var str = "执行存储过程 p_srm_in_agv_task_occupy_place,任务号：" + task + "货位：" + placenum + ",结果" + rtn;
                LogHelper.Info(str);
                return rtn;
            }
            catch (Exception ec)
        ...
        internal static string InOccupyPlacetest(string task, string placenum, string from)
        {
            try
            {
                string rtn = "E初始化未提交数据库";
                ...
                parass.Add(new SqlParameter("@taskcode", task));
                parass.Add(new SqlParameter("@place_code", placenum));
                parass.Add(new SqlParameter("@from", from));
                SqlParameter pa = new SqlParameter();
                pa.Direction = ParameterDirection.InputOutput;
                pa.ParameterName = "@return_value";
                pa.Size = 5000;
