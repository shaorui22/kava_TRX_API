...
{
    public class da_wms_agv_task
    {
        //---------------------按Table的方式查询方法--------------------------//
        private static List<wms_agv_task> DoTableQuery(string sql_str, SqlParameter[] paras)
        {
            var lisRtn = new List<wms_agv_task>();
            try
            {
                var dtback = DBHelper_SQLServer.ExecuteDataTable(sql_str, paras);
                lisRtn = EntityTableParse<wms_agv_task>.ToList(dtback);
            }
            catch
            {
            }
        ...
        private static List<wms_agv_task> DoQuery(string sql_str, SqlParameter[] paras)
        {
            var lisRtn = new List<wms_agv_task>();
            try
            {
        ...
        //----------------------Insert方法----------------------------------------------//
        public static bool InsertNew(wms_agv_task newOne)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(" Insert into wms_agv_task (  ");
            sb.Append("taskid, ");
            sb.Append("taskcode, ");
        ...
        //----------------------UpdateOne方法---------------------------------------//
        public static bool UpdateOne(wms_agv_task newOne)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(" update wms_agv_task set  ");
        ...
        public static bool DeleteByPKID(object PK_ID)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append(" delete from wms_agv_task where taskid=@taskid ");
                SqlCommand cmd = new SqlCommand();
                cmd.CommandText = sb.ToString();
                cmd.CommandType = CommandType.Text;
        ...
        //----------------------GetOneByID方法--------------------------------------//
        public static wms_agv_task GetOneByID(object PK_ID)
        {
            List<SqlParameter> prs = new List<SqlParameter>();
            prs.Add(new SqlParameter("@taskid", SqlDbType.NVarChar, 150) { Value = PK_ID });
            var lis = DoTableQuery("select * from wms_agv_task where taskid =@taskid", prs.ToArray());
        ...
        internal static wms_agv_task GetOneByTaskCode(string taskcode)
        {
            List<SqlParameter> prs = new List<SqlParameter>();
            prs.Add(new SqlParameter("@taskcode", SqlDbType.NVarChar, 150) { Value = taskcode });
            var lis = DoTableQuery("select * from wms_agv_task where taskcode =@taskcode and task_status <> '100' order by t_create desc", prs.ToArray());
            if (lis != null && lis.Count > 0)
            {
                return lis[0];
            }
        ...
        internal static wms_agv_task GetOnedoingByTaskCode(string taskcode)
        {
            List<SqlParameter> prs = new List<SqlParameter>();
            prs.Add(new SqlParameter("@taskcode", SqlDbType.NVarChar, 150) { Value = taskcode });
            var lis = DoTableQuery("select * from wms_agv_task where container_id =@taskcode and (task_status = 'NEW'  or task_status = '30' or task_status = '40')", prs.ToArray());
            if (lis != null && lis.Count > 0)
            {
                return lis[0];
            }
        ...
        internal static wms_agv_task GetDoingPallet(string palletcode, string tasktype)
        {
            List<SqlParameter> prs = new List<SqlParameter>();
            prs.Add(new SqlParameter("@container_id", SqlDbType.NVarChar, 150) { Value = palletcode });
            prs.Add(new SqlParameter("@task_type", SqlDbType.NVarChar, 150) { Value = tasktype });
            var lis = DoTableQuery("select * from wms_agv_task where container_id =@container_id and task_type=@task_type and task_status in ('DOING','NEW' ) and flag_delete=0", prs.ToArray());
            if (lis != null && lis.Count > 0)
            {
                return lis[0];
            }
        ...
        internal static wms_agv_task GetOneByTargetPlace(string toPlaceID)
        {
            List<SqlParameter> prs = new List<SqlParameter>();
            prs.Add(new SqlParameter("@place_to", SqlDbType.NVarChar, 150) { Value = toPlaceID });
            var lis = DoTableQuery("select * from wms_agv_task where place_to =@place_to and flag_delete=0", prs.ToArray());
            if (lis != null && lis.Count > 0)
            {
                return lis[0];
            }
        ...
        internal static List<wms_agv_task> GetNotFinishTasks(string typecode)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("select * from wms_agv_task where task_status <>'FINISHED'  and flag_delete=0");
            if (typecode != "")
            {
        ...
        internal static List<wms_agv_task> GetRecordsByConditions(string mcodeOrdesc, string TimeStart, string TimeEnd, string inout_type)
        {
            StringBuilder sb = new StringBuilder();
            var dtStart = DateTime.Parse(TimeStart);
            var dtEnd = DateTime.Parse(TimeEnd);
            sb.Append("select * from v_wms_agv_task where  CONVERT(varchar(100), t_diliver, 120)>='" + dtStart.ToString("yyyy-MM-dd HH:mm:ss") + "' and CONVERT(varchar(100), t_diliver, 120)<='" + dtEnd.ToString("yyyy-MM-dd HH:mm:ss") + "'");
            if (inout_type != "")
            {
